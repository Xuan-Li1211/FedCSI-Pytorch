...
class DifferentialPrivacy:
    @staticmethod
    def add_laplace_noise(data: np.ndarray, epsilon: float, sensitivity: float, random_state_seed: int) -> np.ndarray:
        """为数据添加拉普拉斯噪声"""
        # 为确保可复现性，每次调用都使用独立的随机状态
        rng = np.random.default_rng(random_state_seed)
        scale = sensitivity / epsilon
        noise = rng.laplace(loc=0.0, scale=scale, size=data.shape)
        return (data + noise).astype(np.float32)

    @staticmethod
    def clip_gradients(model: nn.Module, clip_threshold: float):
        """梯度裁剪"""
...
class ClusteredFedCSIClient:
    def __init__(self, client_id, cluster_id, model, local_features, local_edge_index, local_labels,
                 local_train_indices, node_to_comm_map, local_train_mask, args):
        ...
        self.local_train_mask = local_train_mask # <<< NEW: 保存本地训练掩码
        self.args = args
    ...
    def train(self, global_model_state, global_comm_reps, global_comm_edge_index, new_lr):
        ...
        for _ in range(self.args.local_step):
            ...
            if self.train_idx.numel() > 0:
                ...
                loss.backward()
                # <<< MODIFIED: 在主训练中加入梯度裁剪 >>>
                if self.args.enable_dp:
                    DifferentialPrivacy.clip_gradients(self.model, self.args.grad_clip_threshold)
                self.optimizer.step()
        return self.model.state_dict()

    # <<< MODIFIED: 修复数据泄露, 只返回训练节点的表示 >>>
    @torch.no_grad()
    def generate_initial_reps(self, global_model_state):
        ...
        # 只返回训练集节点的嵌入，以防止信息泄露
        return reps[self.local_train_mask]
...
class ClusteredServer:
    ...
    def initialize_cluster_models(self, num_communities):
        for k in range(self.num_clusters):
            model = self.model_class(nfeat=self.dataset.num_features, nhid=self.args.num_hidden,
                                     nclass=self.dataset.num_classes, dropout=self.args.dropout,
                                     num_layers=self.args.num_layers, num_communities=num_communities).to(self.device)
    ...
    # <<< MODIFIED: train_cluster 修复了数据泄露 >>>
    def train_cluster(self, cluster_id, comm_edge_index, node_to_comm_map, communicate_indexes, full_data_num_nodes):
        ...
        # <<< MODIFIED: 只收集和使用训练节点的表示 >>>
        all_train_reps, all_train_global_ids = [], []
        for client in clients:
            local_train_reps = client.generate_initial_reps(model.state_dict())
            all_train_reps.append(local_train_reps)

            # 找到这些本地训练节点在全局图中的ID
            client_global_nodes = communicate_indexes[client.client_id]
            train_global_ids = client_global_nodes[client.local_train_mask.cpu()]
            all_train_global_ids.append(train_global_ids)

        if not all_train_reps: return None
        ...
        client_model_states = [client.train(model.state_dict(), global_comm_reps, comm_edge_index, new_lr=current_lr)
                               for client in clients]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-nhid", "--num_hidden", default=256, type=int)
    parser.add_argument("-do", "--dropout", default=0.3, type=float)
    ...
    parser.add_argument("-k", "--num_clusters", default=3, type=int)
    parser.add_argument("--fusion_alpha", default=0, type=float)
    parser.add_argument("--enable_dp", action="store_true", default=True, help="是否启用差分隐私保护")
    parser.add_argument("--dp_epsilon", default=1.0, type=float, help="差分隐私预算ε")
    parser.add_argument("--grad_clip_threshold", default=1.0, type=float, help="梯度裁剪阈值")
    ...
    with torch.no_grad():
        for i in tqdm(range(args.n_trainer), desc="生成客户端画像"):
            ...
            if args.enable_dp:
                # 设定一个合理的敏感度上界
                # 假设嵌入和结构特征都被归一化或裁剪到[-1, 1]范围
                sensitivity = 2.0
                profile_vector = DifferentialPrivacy.add_laplace_noise(
                    profile_vector, args.dp_epsilon, sensitivity, random_state_seed=args.seed + i
                )
    ...
    for i in range(args.n_trainer):
        ...
        # 创建本地训练掩码
        client_global_nodes = communicate_indexes[i]
        local_train_mask = torch.zeros(len(client_global_nodes), dtype=torch.bool, device=device)
        # in_com_train_indices[i] 存储的是训练节点在子图中的索引
        local_train_mask[in_com_train_indices[i].to(device)] = True

        client = ClusteredFedCSIClient(
            client_id=i, cluster_id=cluster_id, model=client_model_template,
            local_features=full_data.x[client_global_nodes].to(device),
            local_edge_index=edge_indexes_clients[i].to(device),
            local_labels=full_data.y[client_global_nodes].to(device),
            local_train_indices=in_com_train_indices[i].to(device),
            node_to_comm_map=node_to_comm_map[client_global_nodes],
            local_train_mask=local_train_mask,  # <<< NEW: 传入掩码
            args=args
        )
    ...
    for round_idx in tqdm(range(args.global_rounds), desc="全局训练回合"):
        ...
        for k in range(actual_num_clusters):
            comm_reps_k = server.train_cluster(
                cluster_id=k, comm_edge_index=comm_edge_index, node_to_comm_map=node_to_comm_map,
                communicate_indexes=communicate_indexes, full_data_num_nodes=full_data.num_nodes
            )
        ...
        # <<< MODIFIED: 在计算验证准确率后，为每个簇的调度器执行step
        for k in server.cluster_schedulers:
            server.cluster_schedulers[k].step(val_acc)

        if (round_idx + 1) % 10 == 0:
            print(f"\n回合 {round_idx + 1:03d} | 当前验证准确率: {val_acc:.4f} | 最佳: {best_val_accuracy:.4f} | 早停计数: {patience_counter}/{args.patience}")
    ...
    if any(s is not None for s in best_model_states.values()):
        ...
        # <<< 核心修正：此处的逻辑现在与 train_cluster 完全一致 >>>
        final_comm_info = {}
        with torch.no_grad():
            for k in range(actual_num_clusters):
                clients_in_cluster = server.clients_by_cluster.get(k, [])
                ...
                # 1. 只收集训练节点的嵌入和它们对应的全局ID
                all_train_reps, all_train_ids = [], []
                for client in clients_in_cluster:
                    # generate_initial_reps 现在只返回训练节点的嵌入
                    train_reps = client.generate_initial_reps(model_k.state_dict())
                    all_train_reps.append(train_reps)

                    # 必须同样只获取这些训练节点在全局图中的ID
                    client_global_nodes = communicate_indexes[client.client_id]
                    train_global_ids = client_global_nodes[client.local_train_mask.cpu()]  # 使用掩码筛选
                    all_train_ids.append(train_global_ids)

                # 如果这个簇没有任何训练节点，则跳过
                if not all_train_reps or all(rep.numel() == 0 for rep in all_train_reps):
                    continue

                # 2. 拼接张量，此时维度保证一致
                all_train_reps_cat = torch.cat(all_train_reps, dim=0)
                all_train_ids_cat = torch.cat(all_train_ids, dim=0).to(device)

                # 3. 只基于训练节点计算社区特征
                num_communities = node_to_comm_map.max().item() + 1
                train_comm_map = node_to_comm_map[all_train_ids_cat]
                comm_features = scatter(src=all_train_reps_cat, index=train_comm_map, dim=0, reduce="mean",
                                        dim_size=num_communities)
        ...
        # 5. 使用计算好的 final_comm_info 进行最终评估
        ...
        # 输出隐私保护信息和最终结果
        privacy_info = f"启用 (ε={args.dp_epsilon}, 梯度裁剪={args.grad_clip_threshold})" if args.enable_dp else "禁用"

        print(f"\n=== 最终实验结果 ===")
        print(f"隐私保护设置: {privacy_info}")
